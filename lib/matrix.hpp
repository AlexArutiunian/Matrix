...
public:
    ...
    */
    ...
    int triang_form_Gauss_max(T EPS){
        ...
        for(int i = 0; i != rs; ++i){
            ...
            for(int j = i + 1; j != cs; ++j){
                ...
                // НЕ ДОДЕЛАЛ АЛГОРИТМ!
                // МАКИСМУМ в строке ищется
                // но максимум в столбце нет
    ...
    T make_upper_triangular_square(size_t side_of_square) 
    {
        ...
        {
            auto row_to_swap = find_max_in_col(i);
            if (row_to_swap != i)
            {
                this->fst_E(i + 1, find_max_in_col(i) + 1);
                sign *= T{-1};
            }
            if ((*this)(i, i) > EPS)
            {
                for (size_t j = i + 1; j < side_of_square; j++)
                    for (size_t k = i + 1; k < side_of_square; k++){
                        size_t n = side_of_square;
                        p_m[(j - 1) * n + (k - 1)] = (p_m[(j - 1) * n + (k - 1)] * p_m[(i - 1) * n + (i - 1)] - 
                        p_m[(j - 1) * n + (i - 1)] * p_m[(i - 1) * n + (i - 1)]) / div_coef;
                    }  
                size_t n = side_of_square;      
                div_coef = p_m[(i - 1) * n + (i - 1)];
            }
            else
                sign = 0;
        }
        return (*this)(side_of_square - 1, side_of_square - 1) * sign;
    }
